...
def test_daemon_silent_minutes_is_config_driven(db_session, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        # `handle_alerts` also builds the index_state axis, which reads
        # `system.index_rebuild_pending_threshold` off this same monkeypatched
        # `plugin_config` — include it or that axis's lookup 500s.
        lambda name: SimpleNamespace(daemon_silent_minutes=5, index_rebuild_pending_threshold=50),
    )
    payload = json.loads(system_tools.handle_alerts(db_session, {}))
...
def test_bad_daemon_silent_minutes_falls_back_to_the_constant(db_session, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda name: SimpleNamespace(
            daemon_silent_minutes="not-a-number", index_rebuild_pending_threshold=50,
        ),
    )
    payload = json.loads(system_tools.handle_alerts(db_session, {}))
