...
# ---------------------------------------------------------------------------
# facade (`strava.query` capability — issue #195b, daily-brief Pulse merge)
# ---------------------------------------------------------------------------


def test_manifest_declares_strava_query_capability():
    from app.integrations.strava.manifest import MANIFEST

    assert "strava.query" in MANIFEST.provides


def test_facade_activities_reports_not_connected(db_session):
    """No OAuthToken row at all for this user — must say so distinctly from
    'connected, nothing this week', so the brief renderer can tell the two
    apart (issue #195b)."""
    import json

    from app.integrations.strava.facade import FACADE

    out = json.loads(FACADE.activities(db_session, {"days": 7}))
    assert out == {"connected": False}


def test_facade_activities_lists_recent_activities(db_session):
    import json
    ...
    db_session.add(StravaActivity(
        user_id=1, strava_id=1001, name="Evening Walk", sport_type="Walk",
        activity_type="Walk", start_date=now - timedelta(hours=2),
        distance_m=1830.0, moving_time_s=1368, elapsed_time_s=1400,
    ))
    # Outside the 7-day window — must not appear.
    db_session.add(StravaActivity(
        user_id=1, strava_id=1002, name="Old Ride", sport_type="Ride",
        activity_type="Ride", start_date=now - timedelta(days=30),
        distance_m=20000.0, moving_time_s=3600, elapsed_time_s=3700,
    ))
    db_session.commit()
    ...
    assert out["count"] == 1
    assert out["activities"][0]["name"] == "Evening Walk"
    assert out["activities"][0]["duration_min"] == pytest.approx(1368 / 60.0, abs=0.1)


def test_facade_activities_scoped_to_current_user(db_session):
    """A second athlete's activities must never leak into this user's brief."""
    import json

    from app.auth.context import use_user
    from app.integrations.strava.facade import FACADE

    _token(db_session, expires_in_seconds=3600)  # user_id=1
    other = OAuthToken(
        provider=PROVIDER, user_id=2, account_email="other",
        access_token=encrypt_token("x"), refresh_token=encrypt_token("y"),
    )
    db_session.add(other)
    db_session.add(StravaActivity(
        user_id=2, strava_id=2001, name="Someone Else's Run", sport_type="Run",
        activity_type="Run", start_date=datetime.now(timezone.utc),
        distance_m=5000.0, moving_time_s=1500, elapsed_time_s=1550,
    ))
    db_session.commit()

    with use_user(1):
        out = json.loads(FACADE.activities(db_session, {"days": 7}))
    assert out["connected"] is True  # user 1's own token
    assert out["count"] == 0  # user 2's activity must not leak in
    assert out["activities"] == []


def test_facade_has_data_reflects_any_stored_activity(db_session):
    from app.integrations.strava.facade import FACADE

    assert FACADE.has_data(db_session, 1) is False

    db_session.add(StravaActivity(
        user_id=1, strava_id=3001, name="Run", sport_type="Run",
        activity_type="Run", start_date=datetime(2020, 1, 1, tzinfo=timezone.utc),
        distance_m=1000.0, moving_time_s=300, elapsed_time_s=310,
    ))
    db_session.commit()

    assert FACADE.has_data(db_session, 1) is True
...
def test_callback_is_exempt_from_session_auth():
    """The OAuth return leg must not be gated behind the dashboard session.

    ⚠️ Found only by running the flow, never by the unit tests: the session
    cookie is SameSite=Strict, so when Strava redirects the BROWSER back after
    ...
    assert exempt, "strava OAuth callback must be reachable without a session"


def test_connect_is_NOT_exempt_from_session_auth():
    ...
    exempt = path in AUTH_EXEMPT or any(path.startswith(p) for p in AUTH_EXEMPT_PREFIXES)
    assert not exempt, "strava connect must stay behind the dashboard session"
