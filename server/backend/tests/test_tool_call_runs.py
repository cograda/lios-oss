"""tool-call rows in the `runs` ledger (db tier) — Wave 5.1.

`tool_calls` was a standalone table before Wave 5.1; it is now
`kind="tool_call"` rows in `runs`, written by `app.services.runs.
record_tool_call` (called from `app.plugin.dispatch`, unchanged call
signature). This file covers the same three DB-backed guarantees Phase 5
added on top of the MCP dispatch path (see tests/test_mcp_transport.py for
the auth/dispatch suite this borrows fixtures from), now against `Run`:

  (a) dispatching a real tool through the MCP call_tool path lands exactly
      one `runs` row (kind="tool_call") with a sane duration/outcome
  (b) >3 error-outcome tool_call rows for one tool name in the last hour
      trips the "failing repeatedly" system_alerts check
  (c) a p95 duration over 5000ms (>=10 calls, 24h window) trips the
      "p95 slow" system_alerts check

Scoping between tool_call rows and scheduled_job/manual rows is covered in
tests/test_runs_ledger.py, not here.
"""
...
# ---------------------------------------------------------------------------
# (a) dispatching a tool lands exactly one runs row (kind="tool_call")
# ---------------------------------------------------------------------------

@pytest.mark.anyio
async def test_dispatch_records_one_tool_call_run(mcp_post, tokens, db_session):
    from app.models.runs import Run

    resp = await mcp_post(_call("search_stats"), bearer="alex-client-token")
    assert resp.status_code == 200

    rows = db_session.query(Run).filter(Run.kind == "tool_call", Run.name == "search_stats").all()
    assert len(rows) == 1
    row = rows[0]
    assert row.outcome == "ok"
    assert row.user_id == 1
    assert row.duration_ms >= 0
    assert row.run_id and len(row.run_id) == 8  # secrets.token_hex(4)
    assert row.error_text is None
    assert row.started_at is not None


# ---------------------------------------------------------------------------
# V4 chunk 2.5 (carried over Wave 5.1): audit columns — args_summary/
# trigger(transport)/source_ip on both transports, and the optional
# `affected` field for opted-in write tools.
# ---------------------------------------------------------------------------
...
async def test_mcp_dispatch_records_transport_and_args_summary(mcp_post, tokens, db_session):
    ...
    row = (
        ...
        .filter(Run.kind == "tool_call", Run.name == "search_stats")
...
async def test_snag_add_records_affected(mcp_post, tokens, db_session, tmp_path, monkeypatch):
    ...
    row = (
        db_session.query(Run)
        .filter(Run.kind == "tool_call", Run.name == "snag_add")
        .order_by(Run.id.desc())
        .first()
    )
    assert row is not None
...
# ---------------------------------------------------------------------------
# auth_events — a 401 (either transport) writes a row. Unaffected by the
# tool_calls/runs merge; kept here since these tests share the mcp_post/
# tokens fixtures above.
# ---------------------------------------------------------------------------
...
@pytest.mark.anyio
async def test_dispatch_error_records_error_outcome(mcp_post, tokens, db_session):
    from app.mcp import server as srv
    from app.models.runs import Run
    ...
    try:
        ...
        rows = db_session.query(Run).filter(
            Run.kind == "tool_call", Run.name == "probe_tool_calls_explode"
        ).all()
        assert len(rows) == 1
        assert rows[0].outcome == "error"
        assert "kaboom" in (rows[0].error_text or "")
    finally:
...
def _seed_tool_call(session, *, name, status, duration_ms, called_at, user_id=1, run_id=None):
    import secrets
...
def test_p95_ignores_scheduled_job_rows(db_session):
    """A scheduled_job row with a huge duration must never count toward a
    tool's p95 — kind must be part of the axis-4 filter, not just name.
    Mutation check: dropping `Run.kind == "tool_call"` from the p95 query
    makes this fail, since the 10 slow scheduled_job rows below would then
    contribute to (and trip) `some_job`'s p95 alert."""
    from app.integrations.system.tools import handle_alerts
    from app.models.runs import Run
    ...
    for i in range(10):
        db_session.add(Run(
            run_id=f"sched{i:04d}"[:16], kind="scheduled_job", name="some_job",
            user_id=None, started_at=now - timedelta(hours=1),
            finished_at=now - timedelta(hours=1), duration_ms=9000,
            outcome="ok", trigger="schedule",
        ))
    db_session.commit()

    payload = json.loads(handle_alerts(db_session, {}))
    assert not any(a["tool"] == "some_job" for a in payload["tool_alerts"])
...
def test_a_long_transport_label_is_truncated_not_dropped(db_session):
    ...
    from app.models.runs import Run
    from app.services.runs import record_tool_call

    record_tool_call(
        name="tasks_query", user_id=None, duration_ms=1, status="ok", error=None,
        tool_call_id="tc-truncate", transport="script:backlog-review",
    )
    row = db_session.query(Run).filter_by(run_id="tc-truncate").one()
    assert row.trigger == "script:bac"
