<!-- GENERATED from server/backend/app/prompts/templates/triage.md.j2 via app.prompts.commands — do not hand-edit. Edit the template and run scripts/render_commands.py (or refetch GET /api/v1/commands). -->
...
> - **type**: one of:
>   - `snag` — a defect or move-in job at [[Comar House]] (the house, formerly "Riverside"). Detect by: a `Snag - ...` or `Task - ...` prefix; OR content from Sam / the snagger / a trade describing a fault, unfinished work, or a fitting job in the new house. Sub-classify:
...
3  | email — SSE         | Meter reading due next week             | 🔼       | → backlog
4  | whatsapp — Crannarc | Confirm tile colour before Friday       | ⏫       | → backlog 📅 2026-04-04
...
- **→ snag-list**: **the snag database is the source of truth** (each snag gets a stable `SNAG-NNNN` UID; the vault view `Household/Renovation/Snags.md` regenerates automatically — never hand-edit it).
  ...
  - `comar query - <question>` (Sam addressing the system directly, convention started 2026-07-08) → NOT a snag: add to the **[[Comar Project Board]]** under `## Open queries` as a checkbox with the question and who can answer it (Phil / [[Paddy]] / etc.). Answer directly in the triage summary if comar already knows.
...
- **→ ingest**: Call `attachments_ingest` with `{"ids": [...]}` for the chosen attachment ids. Each becomes a `HistoricalDocument` searchable via `corpus_search`. Confirm count of ingested vs failed.
