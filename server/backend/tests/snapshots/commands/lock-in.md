<!-- GENERATED from server/backend/app/prompts/templates/lock-in.md.j2 via app.prompts.commands — do not hand-edit. Edit the template and run scripts/render_commands.py (or refetch GET /api/v1/commands). -->
...
- **Create calendar event**: For items that are 2+ weeks out or time-specific, offer to create a Google Calendar event via the `calendar_create_event` MCP tool. Examples: "Whelans tickets on sale 10am April 2nd" → calendar event. "Sue's birthday next weekend" → all-day event. Ask the user: "This is far out — want a calendar entry too?"
