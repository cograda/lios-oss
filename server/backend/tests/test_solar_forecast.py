...
class TestSolarForecastDeriver:
    ...
    def algo(self, configured):
        ...
        a = SolarForecastIntegration()
        # Shrink the training-scan window/stride to match what these tests
        # actually write (at most 40 days), instead of the real 60-day/
        # 60-min production default. `training_pairs()` walks every
        # stride-aligned candidate in `train_window_days` regardless of how
        # much history exists — at the production settings that is 60*24=
        # 1,440 candidates per horizon (5,760 across 4 horizons), each
        # costing a real `observe()` round trip to Postgres, even though
        # only ~25-40 days of it ever have data. This was ~28s of the
        # suite's slowest-25 (measured 2026-09-04). Halving the stride and
        # trimming the window to the written range cuts the candidate count
        # by roughly 5x while leaving comfortably more than `min_train_rows`
        # (300) of real, elevation-filtered daylight rows — the same signal,
        # not a smaller sample of it in any way the assertions can see.
        from dataclasses import replace

        a.SPEC = replace(a.SPEC, train_window_days=41, train_stride_min=120)
        return a
