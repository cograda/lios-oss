...
def test_duplicates_backfills_open_tasks_and_reports_pending(ledger, db_session, monkeypatch):
    ...
    out = _call(dupes.tasks_duplicates_handler, db_session, owner="household")
    ...
    again = _call(dupes.tasks_duplicates_handler, db_session, owner="household")
...
def test_distinct_ruling_is_order_independent(ledger, db_session, monkeypatch):
    ...
    assert _call(dupes.tasks_duplicates_handler, db_session, owner="household")["pairs"] == []
...
def test_merge_folds_text_repoints_links_and_drops(ledger, db_session):
    ...
    _call(tasks_block_handler, db_session, owner="household", action="add", uid=other, blocked_by=drop)
    ...
    blocked = _call(tasks_block_handler, db_session, owner="household", action="list")["blocked"]
    assert blocked.get(other) == [keep]
    # And the dropped task has left the open ledger.
    open_uids = {t["uid"] for t in _call(tasks_query_handler, db_session, owner="household")["tasks"]}
    assert drop not in open_uids and keep in open_uids
    # Rows now carry the description, so the app can show notes.
    row = next(t for t in _call(tasks_query_handler, db_session, owner="household")["tasks"] if t["uid"] == keep)
...
def test_duplicates_scoped_to_an_owner_keep_pairs_touching_their_loops(ledger, db_session, monkeypatch):
    """2026-09-06: the Loops summary strip's duplicate count was the
    household's. `owner` keeps pairs where EITHER side is theirs — a duplicate
    between my line and Alex's is the kind neither of us notices, and either
    of us can rule on it — and drops pairs between two of someone else's."""
    from app.auth.context import use_user
    from app.integrations.tasks import dupes
    from app.integrations.tasks.tools import tasks_add_handler

    monkeypatch.setattr("app.services.embedding.EmbeddingService.near_duplicates", lambda *a, **k: [])
    # Alex's pair, both his.
    a = _call(tasks_add_handler, db_session, title="Fit a new lock on the utility-room gate")["created"]["uid"]
    b = _call(tasks_add_handler, db_session, title="Fit the new lock to the gate by the utility room")["created"]["uid"]
    # A cross-person pair: hers against the imported (unowned) boiler line.
    boiler = _uid(db_session, "Book the boiler service")
    with use_user(2):
        c = _call(tasks_add_handler, db_session, title="Book the boiler service this month")["created"]["uid"]
    key = lambda p: tuple(sorted((p["a"], p["b"])))  # noqa: E731

    everyone = _call(dupes.tasks_duplicates_handler, db_session, owner="household")
    assert {key(p) for p in everyone["pairs"]} == {tuple(sorted((a, b))), tuple(sorted((c, boiler)))}

    hers = _call(dupes.tasks_duplicates_handler, db_session, owner="sam")
    assert [key(p) for p in hers["pairs"]] == [tuple(sorted((c, boiler)))]
    assert hers["pair_count"] == 1
    # The index caveat is the whole index's, not hers: any un-indexed open task can pair with hers.
    assert hers["pending_index"] == everyone["pending_index"]

    alex = _call(dupes.tasks_duplicates_handler, db_session, owner="me")
    assert [key(p) for p in alex["pairs"]] == [tuple(sorted((a, b)))]
