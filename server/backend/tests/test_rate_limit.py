...
`record_auth_event` chokepoint for the bearer paths, explicitly for the UI
login route, which doesn't write auth events).
...
class TestUiLoginWiring:
    ...
    def test_under_limit_never_429s(self, monkeypatch):
        ...
        monkeypatch.setattr(settings, "ui_token", "the-real-token")
