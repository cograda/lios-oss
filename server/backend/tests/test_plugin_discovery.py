...
class TestDiscovery:
    def test_discover_integrations_matches_expected(self):
        instances = discover_integrations()
        names = [i.name for i in instances]
        assert set(names) == EXPECTED_INTEGRATION_NAMES
        # No duplicates — the set comparison above can't see a name discovered
        # twice. Derived from the set rather than a literal: the literal read
        # 30 and went stale the day `alerts` landed (lios#230).
        assert len(names) == len(EXPECTED_INTEGRATION_NAMES)
