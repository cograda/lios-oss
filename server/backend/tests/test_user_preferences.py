...
@pytest.fixture
def ui_client(real_db):
    """A TestClient signed in as alex (admin) on the session-gated `/api/*`
    surface — `POST /api/auth/login` with a real bearer, then the cookie.

    2026-09-06: the dashboard signs in a person with their per-user bearer
    and holds a server-side session; there is no shared UI token to pin.
    """
    ...
    with real_db.session() as session:
        row, plaintext = ClientToken.mint(user_id=1, label="prefs-test")
        session.add(row)
        session.commit()

    client = TestClient(app, base_url="https://testserver")  # secure cookie
    resp = client.post("/api/auth/login", json={"token": plaintext})
...
@pytest.mark.db
def test_preferences_route_is_behind_the_session(real_db):
    """The gate is genuinely on: no session cookie → 401 on both verbs."""
    from fastapi.testclient import TestClient
    from app.main import app
