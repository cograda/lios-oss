...
@pytest.fixture
def frozen_embedding_clock(monkeypatch):
    """Pin `app.services.embedding`'s `datetime.now()` to FIXED_NOW.

    R4 (2026-09-04): recency decay computes age from real wall-clock time
    against each chunk's `source_date`/`created_at`. Every fixture in this
    file is seeded at the fixed anchor `FIXED_NOW` (2026-01-15), which
    without this fixture drifts further into the past every day this suite
    runs — score would not just differ from the committed snapshot, it would
    keep changing on every future run, which is exactly the nondeterminism
    this file's fixed-anchor convention exists to avoid.
    """
    ...
    monkeypatch.setattr(emb_module, "datetime", _FrozenDT)
...
class TestObsidianSnapshots:
    ...
    def test_vault_search(self, db_session, stub_embedding_model, frozen_embedding_clock):
        from app.integrations.obsidian.tools import get_mcp_tools
...
class TestWhatsappSnapshots:
    ...
    def test_whatsapp_semantic_search(self, db_session, stub_embedding_model, frozen_embedding_clock):
        from app.integrations.whatsapp.tools import get_mcp_tools
...
class TestGoogleMailSnapshots:
    ...
    def test_gmail_semantic_search(self, db_session, stub_embedding_model, frozen_embedding_clock):
        from app.integrations.google_mail.tools import get_mcp_tools
...
class TestDisabledIntegrationGating:
    ...
    def test_disabled_integration_tools_absent_from_registry(self, real_db, db_session):
        ...
        # test_mcp_transport.py / test_tool_call_runs.py's `mcp_app` fixture).
