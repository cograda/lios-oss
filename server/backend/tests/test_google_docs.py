...
class TestWriteHandler:
    ...
    def test_rewrite_uses_the_creating_account_not_the_caller(self):
        """The owning account is load-bearing, not cosmetic.

        `drive.file` is a per-file grant, so only the token that created a
        document may replace its body. Using the caller's account here would
        fail whenever the other household member did the rewrite.
        """
        export = MagicMock(key="brief", title="Brief", document_id="d1",
                          document_url="u1", owner_account_email="owner@example.invalid")
        owner_token = MagicMock(user_id=99)

        session = MagicMock()
        session.query.return_value.filter_by.return_value.one_or_none.return_value = export
        session.query.return_value.filter_by.return_value.first.return_value = owner_token

        with patch("app.integrations.google_docs.writer.write_markdown") as write, \
             patch("app.integrations.google_docs.tools._account",
                   return_value=("caller@example.invalid", 1)):
            handle_write(session, {"key": "brief", "markdown": "# hi"})

        assert write.call_args.kwargs["owner_user_id"] == 99

    def test_rewrite_reports_a_missing_owner_token_instead_of_guessing(self):
        export = MagicMock(key="brief", owner_account_email="gone@example.invalid")
        session = MagicMock()
        session.query.return_value.filter_by.return_value.one_or_none.return_value = export
        session.query.return_value.filter_by.return_value.first.return_value = None

        with patch("app.integrations.google_docs.writer.write_markdown") as write:
            result = handle_write(session, {"key": "brief", "markdown": "# hi"})

        assert "no stored OAuth token" in result
...
class TestAccountResolution:
    """`_account` picks a Google token, and the direction depends on the call.

    Creating a document prefers the configured household owner so a shared
    doc does not change hands depending on who asked. Reading or editing an
    existing one prefers the *caller*, because the document is usually
    theirs and the owner account may not be able to see it at all.
    """
    ...
    def _session(self):
        """Session whose OAuthToken lookups answer by filter kwargs."""
        session = MagicMock()
        ...
        def query(_model):
            ...
            def filter_by(**kwargs):
                inner = MagicMock()
                if kwargs.get("account_email") == self.OWNER:
                    inner.first.return_value = MagicMock(user_id=7)
    ...
    def _account(self, *, prefer_owner: bool, owner: str | None):
        from app.integrations.google_docs.tools import _account

        with patch("app.plugin.config_store.plugin_config",
                   return_value=MagicMock(docs_owner_account=owner)), \
             patch("app.integrations.google_docs.tools.current_user_id", return_value=3):
            return _account(self._session(), prefer_owner=prefer_owner)

    def test_creating_prefers_the_configured_owner(self):
        assert self._account(prefer_owner=True, owner=self.OWNER)[0] == self.OWNER

    def test_reading_prefers_the_caller(self):
        assert self._account(prefer_owner=False, owner=self.OWNER)[0] == self.CALLER

    def test_creating_falls_back_to_the_caller_when_no_owner_configured(self):
        assert self._account(prefer_owner=True, owner=None)[0] == self.CALLER

    def test_no_usable_account_names_both_possibilities(self):
        from app.integrations.google_docs.tools import _account

        session = MagicMock()
        session.query.return_value.filter_by.return_value.first.return_value = None
        with patch("app.plugin.config_store.plugin_config",
                   return_value=MagicMock(docs_owner_account=None)), \
             patch("app.integrations.google_docs.tools.current_user_id", return_value=3), \
             pytest.raises(PermanentError) as exc:
            _account(session, prefer_owner=False)
        # The message has to say which of the two knobs to reach for.
        assert "docs_owner_account" in str(exc.value)
        assert "connect a Google account" in str(exc.value)


class TestHandlersPassTheRightAccountPreference:
    """Each handler must ask for the account preference its operation needs.

    Testing `_account` in isolation is not enough: a handler passing the
    wrong flag is invisible there, and it is the whole behaviour — a
    `docs_read` that reaches for the household owner's token fails on
    exactly the documents a person is most likely to ask about.
    """

    def _capture(self, handler, arguments, patched_writer_fn):
        seen = {}

        def fake_account(session, *, prefer_owner):
            seen["prefer_owner"] = prefer_owner
            return "acct@example.invalid", 1

        session = MagicMock()
        session.query.return_value.filter_by.return_value.one_or_none.return_value = None
        with patch("app.integrations.google_docs.tools._account", side_effect=fake_account), \
             patch("app.integrations.google_docs.tools._share_with", return_value=[]), \
             patch(f"app.integrations.google_docs.writer.{patched_writer_fn}",
                   return_value={} if patched_writer_fn != "ensure_export" else None):
            handler(session, arguments)
        return seen["prefer_owner"]

    def test_read_prefers_the_caller(self):
        assert self._capture(handle_read, {"document": "d1"}, "read_markdown") is False

    def test_append_prefers_the_caller(self):
        assert self._capture(
            handle_append, {"document": "d1", "text": "x"}, "append_text") is False

    def test_replace_prefers_the_caller(self):
        assert self._capture(
            handle_replace, {"document": "d1", "find": "a"}, "replace_text") is False

    def test_creating_prefers_the_household_owner(self):
        assert self._capture(
            handle_write, {"key": "k", "markdown": "# x"}, "ensure_export") is True
