...
files (one per export batch) and it picks them all up:
    docker compose exec app python -m scripts.ingest_claude_export \\
        --dir /doc_corpus/claude-export-2026-07-26 --owner 1 --tags claude-conversations

`--owner` (a users.id) is required: the export is that one person's history
and is ingested as private to them (2026-09-06 decision). There is no
household-shared Claude export, so there is no default.
"""
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="users.id of the person whose export this is (alex=1, sam=2); the "
             "conversations are searchable by that user only",
    )
