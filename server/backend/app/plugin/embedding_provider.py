...
class GeminiRateLimitError(RuntimeError):
    """A 429/RESOURCE_EXHAUSTED that persisted past
    `rate_limit_max_wait_seconds` of waiting.

    Distinguished from a plain `RuntimeError` on purpose: `_embed_and_store`
    (`app/services/embedding.py`) checks `isinstance` on it to skip the
    primary-space fallback. A 429 is a per-minute quota, not a provider
    outage — falling back to the local `bge-small` model (~1s/item) for it
    is what turned a rate-limited run into one slower than simply waiting
    (the 2026-09-07 incident this class exists to fix). The item is left
    'pending' for the next `*/5` tick instead.
    """


def _retry_after_seconds(exc: Exception) -> float | None:
    """Best-effort Retry-After hint from a Gemini 429.

    The SDK's error shape for this isn't guaranteed stable across versions,
    so every lookup here is opportunistic and this never raises — a miss
    just means the fixed backoff table (`rate_limit_backoff_seconds`)
    applies instead of the server's own hint.
    """
...
class GeminiEmbeddingProvider:
    ...
    # 2026-09-07: a 429/RESOURCE_EXHAUSTED gets its own, much longer wait
    # budget than a genuine failure (`max_retries` above) — see
    # `GeminiRateLimitError`'s docstring for why it never falls back.
    # Bounded so a persistent outage merely wearing a 429 still eventually
    # surfaces as a real failure rather than hanging a tick forever.
    # Configurable via `embedding.gemini_rate_limit_max_wait_seconds`
    # (falls back to this constant); see manifest.py.
    rate_limit_max_wait_seconds = 240.0
    rate_limit_backoff_seconds = (15.0, 30.0, 60.0)  # repeats at 60s past this

    # Estimated token ceiling (chars/4) for one request, on top of the
    # item-count `batch_size` above — a batch of unusually long documents
    # (e.g. email) can burst well past a per-minute quota well before
    # hitting the item-count cap (measured 2026-09-07: ~100k tokens for a
    # 100-item mail batch). Configurable via `embedding.max_batch_tokens`.
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        Chunked by item count (`batch_size`) **and** by an estimated token
        total (`_chunk_by_tokens`) — see `max_batch_tokens`'s comment for why
        the count cap alone isn't enough.
        """
        if not texts:
            return []
        out: list[list[float]] = []
        for chunk in self._chunk_by_tokens(texts):
            out.extend(self._embed_batch(chunk))
        return out

    def _chunk_by_tokens(self, texts: list[str]) -> list[list[str]]:
        """Split `texts` into request-sized groups, respecting both
        `batch_size` (item count) and `max_batch_tokens` (chars/4 estimate).

        A single item over the token budget on its own still goes alone
        (never dropped, never merged) — the check only splits an
        already-nonempty chunk before adding another item.
        """
        ...
        for t in texts:
            ...
            if chunk and (len(chunk) >= self.batch_size or chunk_tokens + t_tokens > max_tokens):
                chunks.append(chunk)
                chunk = []
                chunk_tokens = 0
            chunk.append(t)
    ...
    def _max_batch_tokens(self) -> int:
        # An explicit per-instance override beats the config store. Without
        # this, the db tier (which has a live store carrying the manifest
        # default) silently ignored `p.max_batch_tokens = ...` while the unit
        # tier (no store) honoured it — the same test passed in one tier and
        # ran with the wrong value in the other.
        if "max_batch_tokens" in vars(self):
            return int(self.max_batch_tokens)
        from app.plugin.config_store import plugin_config

        try:
            val = getattr(plugin_config("embedding"), "max_batch_tokens", None)
        except Exception:  # noqa: BLE001 — config lookup must never sink a batch
            val = None
        return int(val) if val else self.max_batch_tokens

    def _rate_limit_max_wait_seconds(self) -> float:
        # Per-instance override first — see `_max_batch_tokens`. Here the
        # cost of getting it wrong was a db-tier test spinning for the full
        # 240s default (with the limiter stubbed out) and CI cancelling the job.
        if "rate_limit_max_wait_seconds" in vars(self):
            return float(self.rate_limit_max_wait_seconds)
        from app.plugin.config_store import plugin_config

        try:
            val = getattr(plugin_config("embedding"), "gemini_rate_limit_max_wait_seconds", None)
        except Exception:  # noqa: BLE001 — config lookup must never sink a batch
            val = None
        return float(val) if val else self.rate_limit_max_wait_seconds
    ...
    def _embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        max_wait = self._rate_limit_max_wait_seconds()
        rate_limit_deadline = time.monotonic() + max_wait
        ...
        while True:
            ...
            try:
                ...
                rate_limited = code == 429 or "RESOURCE_EXHAUSTED" in str(exc)
                ...
                if isinstance(code, int) and 400 <= code < 500 and not rate_limited:
                    raise RuntimeError(
                ...
                if rate_limited:
                    # 2026-09-07: a 429 gets its own wait budget and never
                    # falls back — see `GeminiRateLimitError`'s docstring.
                    remaining = rate_limit_deadline - time.monotonic()
                    if remaining <= 0:
                        _record_embed_usage(
                            self.model_name, contents, started, ok=False,
                            error=f"rate-limited for {max_wait:.0f}s: {str(exc)[:250]}",
                            client=client,
                        )
                        raise GeminiRateLimitError(
                            f"gemini embed still rate-limited after {max_wait:.0f}s "
                            f"— waiting, never falling back to the CPU model for "
                            f"a rate limit: {str(exc)[:300]}"
                        ) from exc
                    hinted = _retry_after_seconds(exc)
                    if hinted is not None:
                        sleep = hinted
                    else:
                        idx = min(rate_limit_attempt, len(self.rate_limit_backoff_seconds) - 1)
                        sleep = self.rate_limit_backoff_seconds[idx]
                        rate_limit_attempt += 1
                    sleep = max(0.0, min(sleep, remaining))
                    ...
                    logger.warning(
                        "gemini embed rate-limited (429) — waiting %.1fs "
                        "(retry-after=%s), not falling back to the CPU model: %s",
                        sleep, hinted,
                        # 300 chars, not 90: truncation hid the decisive part of
                        # an error (quotaId, billing-vs-quota wording) repeatedly.
                        str(exc)[:300],
                    )
                    continue

                # Genuine failure (5xx, network, etc.) — short exponential
                # backoff bounded by `max_retries`; exhausting this one DOES
                # fall back to another space, unlike a rate limit.
