...
class GoogleMailIntegration(SourceIntegration):
    ...
    def store(self, session: Session, records: list[dict]) -> int:
        """Upsert, then enqueue whatever's new for embedding — the mail-side
        mirror of `WhatsAppIntegration.sync()` calling `embed_messages` after
        every sync. See `sync.enqueue_new_mail`'s docstring for why this is
        scoped to exactly the ids just synced rather than the historical
        backlog."""
        count = store_mail(session, records)
        if records:
            from app.integrations.google_mail.sync import enqueue_new_mail

            enqueue_new_mail(
                session,
                [r["google_message_id"] for r in records],
