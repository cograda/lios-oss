...
MANIFEST = IntegrationManifest(
    ...
    config_schema={
        ...
        # integration off via is_configured(), and none of this integration's
        # surface depends on a configured account any more: a caller with no
        # Google token of their own gets a PermanentError at the call site
        # telling them to connect one. See the root CLAUDE.md's "required
        # config is not a fit when read paths still work" rule.
        #
        ...
        # Retired 2026-09-06: no tool reads this any more — every Docs call
        # uses the CALLER's own Google token (Alex's decision that no tool
        # may fall back to another user's credentials; see tools.py). The key
        # stays declared so an existing deployment's config remains valid.
        "docs_owner_account": ConfigFieldSpec(
            type="str", required=False,
            description=(
                "Retired (2026-09-06) — no longer read. Every Docs tool call uses "
                "the calling user's own Google token."
            ),
        ),
