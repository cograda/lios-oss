...
class VisionFacade:
    ...
    def compare(
        ...
        question: str,
        *,
        role: str = "vision.watch",
        order: str = "in the order given",
    ) -> dict:
        """Ask a yes/no-with-confidence question about a sequence of images.

        First caller: `app/integrations/signals/watchers/base.py`'s vision
        check. Returns `{"answer": bool, "confidence": float, "where":
        str|None, "notes": str}` always — parsing failures on the model's
        side degrade to `answer=False, confidence=0.0` rather than raising,
        same "never raises" stance as `describe()`, because this gates a
        scheduled job, not an interactive request.
        """
        from app.integrations.vision import client as _client
        from app.services import ai_roles
        ...
        api_key = (cfg.gemini_api_key or "").strip()
        if not api_key:
            return {"answer": False, "confidence": 0.0, "where": None,
                     "notes": "no gemini_api_key configured"}

        try:
            binding = ai_roles.resolve_vision_watch(
                fallback=MANIFEST.config_schema["watch_model"].default
            )
        except ai_roles.RoleNotBoundError as exc:
            logger.error("[vision] role 'vision.watch' is not bound: %s", exc)
            return {"answer": False, "confidence": 0.0, "where": None, "notes": str(exc)[:300]}

        try:
            result = _client.compare(
                images, question,
                api_key=api_key, model=binding.model, order=order,
                max_image_mb=cfg.max_image_mb or 18,
            )
            result["model"] = binding.model
            return result
        except ComarError as exc:
            logger.warning("[vision] compare() failed: %s", exc)
            return {"answer": False, "confidence": 0.0, "where": None, "notes": str(exc)[:300]}
        except Exception as exc:  # noqa: BLE001
            logger.exception("[vision] compare() raised unexpectedly")
            return {"answer": False, "confidence": 0.0, "where": None, "notes": str(exc)[:300]}
