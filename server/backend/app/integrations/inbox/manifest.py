...
MANIFEST = IntegrationManifest(
    ...
    # kernel setting. There is deliberately no `inbox_token` here any more
    # (removed 2026-09-06): the ingest route authenticates with the per-user
    # `client_tokens` bearer only — lios has one credential — so this
    # integration owns no shared webhook secret. A stale `inbox_token` row in
    # `integration_config` is ignored by `plugin_config` (unknown keys are
    ...
    # `inbox.query` (lios#159): `tasks_intake_candidates` reads a caller's own
    # pending captures (voice memos, Shortcuts text, described photos, routed
    # WhatsApp self-notes) as an intake source, via `facade.pending_candidates`.
    # Safe in this direction only — see facade.py's module docstring.
    provides=["inbox.query"],
)
