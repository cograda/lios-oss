...
# Vision's sibling of the above (issue #147): `describe_pending` used to stamp
# `described_at` on the very first attempt regardless of outcome, so a
# provider error/timeout/429 buried an image's description forever with no
# retry — the exact bug `MAX_TRANSCRIPTION_ATTEMPTS` already exists to avoid
# for audio. Same cap, same "give up and record it" ending.
MAX_VISION_ATTEMPTS = 3

# Minutes to wait since the last failed attempt before trying again, indexed
# by how many attempts have already happened (1st failure -> wait this long
# before the 2nd try, 2nd failure -> wait this long before the 3rd/last try).
# Vision's sweep runs hourly, so this only matters if the interval is ever
# tightened — without it a provider outage could burn the whole retry budget
# inside the same hour it started.
VISION_RETRY_BACKOFF_MINUTES = [10, 60, 360]
...
#
# `%PDF` and `PK\x03\x04` are deliberately NOT in this list even though both
# are document-ish: `app.services.doc_sniff.sniff_document_kind()` is
# checked ahead of this table (see `sniff_kind()` below) and owns both,
# because a `PK\x03\x04` zip needs its member list inspected to tell a
# generic zip from docx/xlsx (issue #140) — a plain prefix match can't do
# that, and duplicating the check here would be the second sniffer this
# module's own docstring-lesson warns against.
_MAGIC = [
    (b"\x89PNG\r\n\x1a\n", "image"),
...
def sniff_kind(path: Path) -> str:
    ...
    video | html | docx | xlsx | doc | xls | zip | unknown.

    `doc`/`xls` are legacy MS-CFB (OLE2) files — recognised, not parsed.
    `docx`/`xlsx` are content-verified (the zip's member list, not merely its
    `PK\\x03\\x04` prefix) even when the extension already says so and even
    when it doesn't: nothing arriving via the inbox webhook has a trustworthy
    filename (see the ftyp/HTML fixes above), and issue #140 is the same
    lesson for Office documents — a `.doc` that is really a zip must route
    on its real bytes, not its extension. The document-format part of this
    (pdf/doc/xls/docx/xlsx/zip) is `app.services.doc_sniff.sniff_document_kind`
    — shared with `historical_corpus` and `attachments`, which have the same
    problem for a filename extension and a sender-declared mime_type
    respectively, so there is exactly one place this logic is written.
    """
    ...
    if ftyp_kind:
        return ftyp_kind
    doc_kind = sniff_document_kind(path)
    if doc_kind != "unknown":
        return doc_kind
...
def _notify_document_email(
    ...
    **Why this exists.** Audio always had a second beat: a transcript arrives
    ...
    `owner_user_id`/`filed`/`require_content` (added for lios#198/#191's
    `metadata.comar=false` path, `_email_only_document`): `owner_user_id`
    overrides the path-derived owner for a file that was never written under
    `inbox_root()` and so has no owner `owner_user_id_from_path` can find;
    `filed=False` drops the "still on disk" claim, since the email-only path
    deletes its scratch file right after this call; `require_content=False`
    still sends when there's nothing extracted, since there is no push to
    have already said something — silence there would mean the caller hears
    nothing at all about what they just sent.
    """
    owner = owner_user_id if owner_user_id is not None else owner_user_id_from_path(path)
    if owner is None:
    ...
    if not content:
        if not require_content:
            ...
            # Nothing to say beyond what the push already said. An email whose
            # body is a filename is worse than no email — it trains you to
            # ignore them.
    ...
    footer += f"\nStill on disk at {path}" if filed else '\nNot filed — "file in Comar?" was answered No.'

    attachment = _original_file_attachment(path, meta)
    if attachment is None:
        footer += "\n(Original not attached — too large.)"

    intro = "A file was captured to your inbox." if filed else "A file was captured but not filed."
    body_html = _capture_email_html("Document Captured", intro, section, content, footer=footer)
    body_text = f"{intro}\n\n{content}\n\n{footer}\n"
...
def _notify_transcript_success_email(
    meta: dict[str, Any], path: Path, transcript: str,
    *, owner_user_id: int | None = None, filed: bool = True,
) -> None:
    ...
    `owner_user_id`/`filed` (lios#198/#191's `metadata.comar=false` path,
    `_email_only_transcribe`): `owner_user_id` overrides the path-derived
    owner, since a spooled email-only capture lives outside `inbox_root()`
    and has no owner `owner_user_id_from_path` can resolve; `filed=False`
    only changes the intro line, noting the memo was never saved.
    """
    owner = owner_user_id if owner_user_id is not None else owner_user_id_from_path(path)
    if owner is None:
    ...
    intro = (
        "Your transcript has been processed." if filed
        else 'Your transcript has been processed — not filed ("file in Comar?" was answered No).'
    )
    body_html = _capture_email_html("Transcript Complete", intro, "Transcript", transcript)
    body_text = f"{intro}\n\n{transcript}\n"
...
def _notify_transcript_failure_email(
    ...
    *, owner_user_id: int | None = None, filed: bool = True,
    ...
    `owner_user_id`/`filed` (lios#198/#191's `metadata.comar=false` path,
    `_email_only_transcribe`): `owner_user_id` overrides the path-derived
    owner for a spooled file with no `inbox_root()`-relative owner;
    `filed=False` means the recording genuinely will not survive this call
    (its scratch file is deleted right after), so the "still on disk"
    claim and the one-attempt vs three-attempt wording both flip, and the
    original is attached here instead — for a filed capture the audio
    stays in the inbox and re-attaching it would be redundant, but for an
    email-only one this attachment is the only surviving copy.
    """
    owner = owner_user_id if owner_user_id is not None else owner_user_id_from_path(path)
    if owner is None:
    ...
    attempts_phrase = "after three attempts" if filed else "on this attempt"
    if filed:
        closing = (
        ...
        closing = (
            'This was captured with "file in Comar?" answered No, so nothing '
            "was kept on the server — the original is attached here if it "
            "could still be read, otherwise it will need to be re-recorded."
        )

    body_text = (
        f"No usable transcript was produced {attempts_phrase}.\n\n"
    ...
    body_html = (
        ...
        f"<p>No usable transcript was produced {attempts_phrase}.</p>"
        f"<table cellpadding='0' cellspacing='0'>{rows}</table>"
        f"<p style='color:#4a4a4a;'>{_html.escape(closing)}</p>"
        "</div>"
    )
    ...
    try:
        ...
        if not filed:
            ...
            attachments = [attachment] if attachment else None
        get_capability("notify.email").send_email(
            owner, subject, body_html, body_text, attachments=attachments,
        )
    except Exception:  # noqa: BLE001
...
# ---------------------------------------------------------------------------
# metadata.comar=false — email-only capture, never filed (lios#198/#191)
# ---------------------------------------------------------------------------
#
# "File in Comar? Yes/No" answered No. Everything below runs entirely
# outside the inbox tree: no `InboxItem` row, no sidecar, no push. The only
# durable effect is an email to whoever captured it.


def spool_capture(file_bytes: bytes, original_filename: str | None) -> tuple[Path, str]:
    """Write `file_bytes` to a scratch file OUTSIDE `inbox_root()` and sniff
    its kind.

    `sniff_kind`/`extract_preview`/transcription all want a real `Path`, so
    the bytes have to land on disk somewhere — but nothing about this path
    may be `inbox_root()`, since that is exactly the tree this whole
    branch exists to stay out of. `tempfile.mkdtemp()` guarantees that by
    construction rather than by convention (a sibling directory under
    `/inbox` would still need someone to remember never to point a sweeper
    at it).

    The caller (`deliver_capture_by_email`, always run as a
    `BackgroundTasks` entry) deletes the returned directory when it is
    done — nothing written here is meant to outlive that one call.
    """
    import tempfile

    tmp_dir = Path(tempfile.mkdtemp(prefix="lios-inbox-email-only-"))
    ext = Path(original_filename or "").suffix
    path = tmp_dir / f"capture{ext}"
    path.write_bytes(file_bytes)
    return path, sniff_kind(path)


def deliver_capture_by_email(
    path: Path, *, owner_user_id: int, original_filename: str | None, kind: str,
) -> None:
    """Background task: turn a spooled `metadata.comar=false` capture into
    one email to its owner, then remove every trace of it.

    `path` is what `spool_capture` wrote. Regardless of outcome — success,
    failure, or an exception this function itself didn't anticipate — the
    `finally` block below removes `path`'s parent directory, so nothing
    from this call ever remains on disk. There is deliberately no sweeper
    or retry cron for this path the way `transcribe_pending` is for filed
    audio: there is no persisted item for a sweep to find, by design, so a
    transient failure here is reported once rather than requeued.
    """
    import shutil

    try:
        if kind in ("audio", "video"):
            _email_only_transcribe(
                path, kind, owner_user_id=owner_user_id, original_filename=original_filename,
            )
        else:
            _email_only_document(
                path, kind, owner_user_id=owner_user_id, original_filename=original_filename,
            )
    except Exception:  # noqa: BLE001
        logger.exception(
            f"[inbox] email-only delivery failed for {original_filename or path.name}"
        )
    finally:
        shutil.rmtree(path.parent, ignore_errors=True)


def _email_only_transcribe(
    path: Path, kind: str, *, owner_user_id: int, original_filename: str | None,
) -> None:
    """Transcribe a spooled email-only capture and email the result.

    One attempt, not three: `transcribe_file`'s retry budget
    (`MAX_TRANSCRIPTION_ATTEMPTS`) exists because a filed memo survives to
    be retried by the next `*/5` sweep. This file will not exist by the
    time any sweep could run, so there is nothing a retry here would do
    differently — it either works now or it gets reported as failed now.
    """
    from app.plugin.capabilities import get_capability

    _preview, extra = extract_preview(path, kind)
    meta: dict[str, Any] = {
        "kind": kind, "original_filename": original_filename, "preview_meta": extra,
    }

    transcription = get_capability("transcription.audio")
    if not transcription.available():
        logger.info(
            f"[inbox] email-only capture ({original_filename or path.name}): "
            "transcription unavailable"
        )
        _notify_transcript_failure_email(meta, path, owner_user_id=owner_user_id, filed=False)
        return

    result = transcription.transcribe(
        path,
        vault_root=_vault_root_for_dictionary(owner_user_id),
        duration_s=extra.get("duration_seconds"),
    )
    if result.title:
        meta["title"] = result.title

    if result.text:
        _notify_transcript_success_email(
            meta, path, result.text, owner_user_id=owner_user_id, filed=False,
        )
    else:
        _notify_transcript_failure_email(meta, path, owner_user_id=owner_user_id, filed=False)


def _email_only_document(
    path: Path, kind: str, *, owner_user_id: int, original_filename: str | None,
) -> None:
    """Describe a spooled email-only non-audio capture and email it.

    Enrichment here is the same synchronous, local, non-billable extraction
    the filed pipeline runs inline at ingest (`extract_preview`) — nothing
    here waits on a paid sweep the way images do in the filed path, so
    there is no reason to defer this one.
    """
    preview, extra = extract_preview(path, kind)
    meta: dict[str, Any] = {
        "kind": kind, "original_filename": original_filename, "preview": preview, **extra,
    }
    _notify_document_email(meta, path, owner_user_id=owner_user_id, filed=False, require_content=False)
...
def _vision_backoff_minutes(attempts: int) -> int:
    """Minutes to wait since the last attempt before trying attempt `attempts + 1`.

    `attempts` is the number of failures already recorded (>= 1 by the time
    this is consulted — see `_vision_ready_for_retry`). Clamped to the last
    entry so a future bump of `MAX_VISION_ATTEMPTS` doesn't index past the
    list.
    """
    idx = min(attempts - 1, len(VISION_RETRY_BACKOFF_MINUTES) - 1)
    return VISION_RETRY_BACKOFF_MINUTES[max(idx, 0)]


def _is_legacy_vision_failure(meta: dict[str, Any]) -> bool:
    """Whether `meta` is a pre-#157 permanent-failure sidecar (issue #160).

    The retry logic added by #147 never produces `described_at` set
    alongside `vision_error` set, `vision_attempts` unset/zero, *and*
    `vision_failed` unset — a real failure always increments
    `vision_attempts` (or, on the last allowed attempt, pops it and sets
    `vision_failed` instead). But the code #147 replaced stamped
    `described_at` on the very first attempt regardless of outcome, so a
    row a provider errored on before that fix carries exactly this
    combination: `described_at` set, `vision_error` a real error, `note`
    empty, `vision_attempts` 0. That's not a terminal outcome, it's a
    never-actually-retried failure — reclassify it as eligible for one
    retry rather than a permanent skip.

    Both success paths in `describe_pending` clear `vision_error`, so a
    row that has genuinely been re-described since can't be misread as
    legacy again on a later sweep.
    """
    return (
        bool(meta.get("vision_error"))
        and not meta.get("vision_failed")
        and not meta.get("vision_attempts")
    )


def vision_described(meta: dict[str, Any]) -> bool:
    """Whether vision has reached a genuine terminal outcome for `meta`.

    Not simply `bool(meta.get("described_at"))`: a legacy pre-#157 sidecar
    (issue #160) also has `described_at` set but is due another attempt —
    see `_is_legacy_vision_failure`. Callers that report "described" to a
    human (`inbox_pending`, `inbox_preview`) should use this instead of
    reading `described_at` directly, or a legacy row reads as "looked at,
    nothing to say" forever.
    """
    return bool(meta.get("described_at")) and not _vision_ready_for_retry(meta)


def _vision_ready_for_retry(meta: dict[str, Any], *, now: datetime | None = None) -> bool:
    """Whether a not-yet-described image is due another vision attempt.

    The retry-selection rule from issue #147: never described (`described_at`
    unset — a terminal outcome, success or permanent give-up, always sets
    it), fewer than `MAX_VISION_ATTEMPTS` prior failures, and — once it has
    failed at least once — long enough since the last attempt that a
    transient provider error doesn't get hammered again on the very next
    sweep tick. Issue #160: a legacy pre-#157 failure also has `described_at`
    set but isn't a terminal outcome — `_is_legacy_vision_failure` carves
    that case back out so it gets its one retry.
    """
    if meta.get("described_at") and not _is_legacy_vision_failure(meta):
        return False
    attempts = int(meta.get("vision_attempts") or 0)
    if attempts >= MAX_VISION_ATTEMPTS:
        return False
    if attempts == 0:
        return True

    last_attempted = meta.get("vision_attempted_at")
    if not last_attempted:
        # Attempts recorded with no timestamp shouldn't happen (both are
        # always written together below) — retry rather than getting a file
        # permanently stuck on a data shape that should be impossible.
        return True
    try:
        last = datetime.fromisoformat(last_attempted)
    except ValueError:
        return True

    now = now or datetime.now(timezone.utc)
    elapsed_minutes = (now - last).total_seconds() / 60
    return elapsed_minutes >= _vision_backoff_minutes(attempts)
...
def describe_pending(limit: int = 20) -> dict[str, int]:
    ...
    **Bounded retry (issue #147).** This used to stamp `described_at` on the
    very first attempt regardless of outcome — a provider error, timeout, or
    429 buried an image's description forever with nothing ever trying again,
    because the idempotency check below skips anything with `described_at`
    set. Now a failed attempt (a `VisionResult` carrying an `error`) leaves
    `described_at` unset and records `vision_attempts` + `vision_attempted_at`
    instead, so `_vision_ready_for_retry` picks it back up on a later sweep —
    after `MAX_VISION_ATTEMPTS` failures it gives up for good, stamps
    `described_at`, and sets `vision_failed` so that's visible to
    `inbox_pending`/`inbox_preview` rather than looking identical to "not
    looked at yet". A **genuine** empty result (no error — a safety block, or
    an image with nothing to read) is still terminal on the first attempt,
    same as `transcribe_file`'s equivalent case, because retrying can't
    change what the model already looked at and reported nothing on.

    `limit` bounds spend per sweep: a retried attempt still costs a paid
    call, so it counts against the cap the same as a first attempt — a folder
    of holiday photos (or a flaky provider) walks through over successive
    sweeps instead of issuing unbounded paid calls at once.
    """
    from app.plugin.capabilities import get_capability
    ...
    counts = {
        "considered": 0, "described": 0, "skipped": 0, "failed": 0,
        # A failed attempt that will be retried on a later sweep — kept
        # separate from "failed" (permanent give-up) the same way
        # `transcribe_pending` distinguishes them.
        "retrying": 0,
        # Not yet due another attempt (still inside its backoff window) —
        # distinct from "skipped" (already terminal), so a log line doesn't
        # read "nothing to do" when there's really a queue waiting on a
        # provider outage to clear.
        "deferred": 0,
    }
    ...
    for path in iter_all_pending_files():
        if counts["described"] + counts["failed"] + counts["retrying"] >= limit:
            break
        ...
        counts["considered"] += 1
        ready = _vision_ready_for_retry(meta)
        if meta.get("described_at"):
            if not ready:
                counts["skipped"] += 1
                continue
            # A legacy pre-#157 failure (issue #160): `described_at` is set
            # but `ready` says otherwise — fall through for its one retry.
        elif not ready:
            counts["deferred"] += 1
            continue
        ...
        meta["vision_attempted_at"] = datetime.now(timezone.utc).isoformat()
        if result.model:
        ...
        if result.ok:
            ...
            meta["described_at"] = meta["vision_attempted_at"]
            meta.pop("vision_attempts", None)
            meta.pop("vision_failed", None)
            # Issue #160: a stale `vision_error` from an earlier failed
            # attempt would otherwise make this row look like a legacy
            # failure (`_is_legacy_vision_failure`) forever.
            ...
            # Genuine silence: the model reported nothing but no error either
            # — retrying wouldn't change that, so this is terminal now, not a
            # failure to retry.
            meta["described_at"] = meta["vision_attempted_at"]
            meta.pop("vision_attempts", None)
            meta.pop("vision_failed", None)
            meta.pop("vision_error", None)  # see issue #160 note above
            counts["described"] += 1
        else:
            meta["vision_error"] = result.error[:300]
            attempts = int(meta.get("vision_attempts") or 0) + 1
            if attempts >= MAX_VISION_ATTEMPTS:
                meta["described_at"] = meta["vision_attempted_at"]
        ...
        if result.ok:
            ...
            _notify_document_email(meta, path)
        elif meta.get("vision_failed"):
            _notify_enriched(
                meta, path, "lios: image description failed — try again",
                severity="warning",
            )

    if counts["described"] or counts["failed"] or counts["retrying"]:
...
def list_pending(user_id: int, limit: int = 50) -> list[dict[str, Any]]:
    ...
    ⚠️ **Takes the `limit` most recent files, not the first `limit` of
    `iter_pending_files`'s oldest-first order (lios#184).** That function
    sorts ascending on purpose — the background sweeps want to work a
    backlog oldest-first — but slicing an ascending list with `[:limit]`
    keeps the *oldest* `limit` items. With `inbox_pending`'s default
    `limit=20`, any user whose pending backlog exceeds 20 files had every
    capture past the 20th-oldest silently excluded from every call, forever
    (until enough older items were cleared): a file confirmed captured by
    the ingest route never appeared here, with no error and no sign a
    backlog even existed. Slicing from the tail instead guarantees a fresh
    capture is always visible; the returned page is still oldest-first
    among itself, since `iter_pending_files`'s order is preserved."""
    if user_id == LEGACY_OWNER_USER_ID:
        adopt_legacy_files()
    root = user_root(user_id).resolve()
    now = datetime.now(timezone.utc)
    out: list[dict[str, Any]] = []
    pending = iter_pending_files(user_id)
    page = pending[-limit:] if limit > 0 else ([] if limit == 0 else pending)
    for p in page:
        meta = read_sidecar(p)
        st = p.stat()
        try:
        ...
        out.append({
            ...
            # ISO timestamp alongside `age_minutes` above — `tasks_intake_candidates`
            # (lios#159) windows inbox candidates the same way it windows every
            # other source, and a minute-granularity age can't be compared
            # against an arbitrary `since`/`until` boundary.
            ...
            # `described` says the sweep reached this file *and terminally
            # finished with it*; `vision_error` says what happened. Neither
            # was previously visible to any caller. Uses `vision_described`,
            # not a bare `described_at` check, so a legacy pre-#157 failure
            # (issue #160) still due a retry doesn't read as "looked at,
            # nothing to say".
            "described": vision_described(meta),
            "vision_error": meta.get("vision_error"),
            # Issue #147: a failed vision attempt now retries (up to
            # `MAX_VISION_ATTEMPTS`) rather than being silently permanent, so
            # a caller needs to tell "still retrying" (`vision_attempts` set,
            # `described` False) from "gave up for good" (`vision_failed`
            # True) from "hasn't been tried yet" (both unset).
