...
def handle_pending(session: Session, arguments: dict[str, Any]) -> str:
    ...
    for p in scan.iter_pending_files(user_id):
        try:
            scan.enrich_one(p)
...
def handle_preview(session: Session, arguments: dict[str, Any]) -> str:
    ...
    if kind == "pdf":
        ...
        # there is free and, crucially, never re-bills a Gemini call: vision
        # records `described_at` even on failure precisely so a file is not
        # re-sent on every sweep, so re-describing here would defeat that.
        #
        ...
        if not image_text:
            # Three different situations, and a caller has to be able to tell
            # them apart — "not looked at yet" is retryable, "failed" is
            # reportable, and "no text in the image" is a finished answer.
            if not meta.get("described_at"):
                payload["hint"] = (
...
def handle_to_corpus(session: Session, arguments: dict[str, Any]) -> str:
    ...
    # (Tines often does this), we need to hand it a path with the right
