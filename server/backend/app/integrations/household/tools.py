...
detail and `models.py` for the scoping contrast with Domain above):
  ...
  - household_capture_capture: scan WhatsApp for task/nag/discuss/surface-
    shaped messages and register them (idempotent). Sends a best-effort
    push via `notify.push` for each newly created capture — see that
    handler's docstring for what "confirmation" actually means today, given
    the WhatsApp bridge itself cannot send a reply.
...
from app.integrations.household.render import render_domains_note
...
def household_capture_capture_handler(session: Session, arguments: dict) -> str:
    ...
    if result["created_items"]:
        ...
        for item in result["created_items"]:
            _notify_capture_confirmation(
                item["kind"], item["capture_text"], users_by_id.get(item["sender_user_id"]),
            )
...
def household_capture_review_handler(session: Session, arguments: dict) -> str:
    ...
    for raw_id in ids:
        capture = session.get(HouseholdCapture, int(raw_id))
        if capture is None:
            continue
    ...
    session.commit()
    return json.dumps({"reviewed": reviewed_ids, "not_found": [i for i in ids if int(i) not in reviewed_ids]})
    ...
    return [
        ...
        CustomTool(
            ...
            description=(
                ...
                "drawer' or 'Surface: bins') and register them in the "
                "capture inbox, attributed to whoever sent them. Which "
                "keywords are watched for is deployment config (default "
                "task/discuss/surface; 'nag' is available but off by "
                "default — see the household integration's capture_"
                "keywords config field). Idempotent — already-captured "
                "messages are skipped, and a message whose content matches "
                "an existing capture (however it was created) is linked "
                "rather than duplicated. Best-effort pushes a confirmation "
                "notification to the capturer's own device for each new "
                "capture."
            ),
            input_schema={
                "type": "object",
        ...
        CustomTool(
            ...
            description=(
                "Manually add a captured item — task, nag, discuss, or "
                "surface — typed directly or transcribed from voice. Lands "
                "in the capture inbox for later review, not directly in "
                "Task Backlog.md. Defaults the sender to the caller; pass "
                "'sender' to attribute it to someone else (e.g. entering a "
                "nag on their behalf)."
            ),
            input_schema={
                "type": "object",
