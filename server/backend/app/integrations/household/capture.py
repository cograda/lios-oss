"""Capture `task`/`nag`/`discuss`/`surface`/`feedback`-shaped WhatsApp
messages into the household capture inbox — Phase A2 of
vault/Projects/lios/Plans/household-ops-and-loops-2026-08.md.

`feedback` (Wave 2 N4) is the odd one out among the five: it isn't
addressed to a person who does something about the *content* the way a
`nag`/`task`/`surface` is — it's a report that something in the system
itself ("this is hard / flaky / broken") should go to whoever triages the
system, which today means Alex. See `tools.py::_notify_feedback_recipient`
and the `household` manifest's `feedback_recipient_user` config key for the
routing; this module only needs to know it's a valid keyword.
...
# call themselves.
KINDS: tuple[str, ...] = ("task", "nag", "discuss", "surface", "feedback")
...
# why `nag` is excluded by default. `feedback` IS in the default: unlike
# `nag`, there's no social-risk open question attached to it (Wave 2 N4) —
# a message starting "feedback: the loops app is slow" from Sam's own
# self-chat is exactly the intended capture path, and it should work out of
# the box, not require a deliberate config PUT first.
DEFAULT_CAPTURE_KEYWORDS: tuple[str, ...] = ("task", "discuss", "surface", "feedback")
