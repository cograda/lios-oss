...
"discuss", "surface"]`. **`nag` is deliberately excluded from the
default** — household-ops-and-loops-2026-08.md's open decision 6 flags it
as socially risky and explicitly undecided ("Are nag and surface both in
...
themselves; this key only gates the unattended WhatsApp scan.
"""
...
MANIFEST = IntegrationManifest(
    ...
    description=(
        ...
        "surface-shaped messages awaiting review (which keywords the "
        "WhatsApp scan watches for is configurable; see capture_keywords). "
        "DB is the source of truth; no scheduled sync."
    ),
    icon="Home",
    ...
    config_schema={
        "capture_keywords": ConfigFieldSpec(
            type="list_str",
            default=["task", "discuss", "surface"],
            description=(
                "Which capture keywords the WhatsApp scan (household_"
                "capture_capture) watches for, leading a message ("
                "'Nag, Tupperware drawer', 'Surface: bins'). Must be a "
                "subset of task/nag/discuss/surface. Default excludes "
                "'nag': household-ops-and-loops-2026-08.md's open "
                "decision 6 flags nag as socially risky and undecided, "
                "so enabling it here is a deliberate household choice, "
                "not a build. Does not affect household_capture_add "
                "(manual/typed/voice capture), which accepts any kind."
            ),
        ),
    },
    oauth=None,
