...
def _chunk_metadata(modified_iso: str, heading_path, status: str | None) -> str:
    ...
    *filterable* without being *embedded*: putting "status: active" into the
    ...
    """
    meta = {"modified_at": modified_iso, "heading_path": heading_path}
    if status:
        meta["status"] = status
...
def index_vault(session: Session, vault_path: str, user_id: int) -> dict:
    ...
    if to_enqueue:
        ...
        for rel_path, versioned_hash, content, mtime in to_enqueue:
            ...
            for c in chunks:
                metadata = _chunk_metadata(mtime.isoformat(), c.heading_path, status)
...
def index_single_file(
    ...
    status = frontmatter_status(content)
    for c in chunks:
        metadata = _chunk_metadata(now.isoformat(), c.heading_path, status)
        EmbeddingService.enqueue(
            session, "vault", c.source_id, c.text, metadata, user_id=user_id,
        )
