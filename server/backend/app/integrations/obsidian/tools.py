...
def _folder_prefix(session: Session, query, value: str):
    ...
    if not value:
        return query
    return query.filter(
        VaultChunk.path.ilike(f"{escape_ilike(value)}%", escape=ILIKE_ESCAPE_CHAR)
    )
...
def _vault_stats_compute(session: Session, _arguments: dict[str, Any]) -> dict:
    ...
    total_tracked = (
        session.query(func.count(VaultChunk.id))
        .filter(VaultChunk.user_id == uid).scalar() or 0
    )
    total_embedded = (
        session.query(func.count(Embedding.id))
        .filter_by(source="vault", user_id=uid).scalar() or 0
    )
    latest = (
        session.query(func.max(VaultChunk.indexed_at))
        .filter(VaultChunk.user_id == uid).scalar()
    )
    ...
    queue_pending = (
        ...
        .filter_by(source="vault", status="pending", user_id=uid).scalar() or 0
    )
    ...
    return [
        CustomTool(
            ...
            description=(
                ...
                "note matching in nine is about the subject. Optionally filter by folder "
                "or status. "
