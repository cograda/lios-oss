...
def _handle_semantic_search(session, arguments: dict) -> str:
    ...
    # R4 decay decision: left ON (the default) even though this spans sources
    # where recency is deliberately not a signal (historical_corpus, coffee —
    # see their own handlers' opt-out comments). A single cross-source query
    # has no per-source knob to give here; documented as a known limitation
    # rather than fixed now, same shape as the date-range note above (a
    # cross-source aggregator's filters don't apply uniformly to every
    # source, and that's disclosed via `note` rather than silently patched
    # per source).
