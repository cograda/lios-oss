...
# 0.82–0.84 — two lines about the same FNIRSI return, two about the same gate
# lock — mixed with related-but-distinct pairs at the same scores. A person
# rules on both kinds in seconds; a threshold that hides the first kind to
# spare them the second is the wrong trade.
...
def duplicate_pairs(session: Session, threshold: float = DEFAULT_THRESHOLD, limit: int = 60) -> list[dict]:
    from app.services.embedding import EmbeddingService
    ...
    open_list = session.query(Task).filter(Task.status.in_(OPEN_STATUSES)).all()
...
def similar_tasks(session: Session, uid: str, limit: int = 8) -> list[dict]:
    ...
    open_list = session.query(Task).filter(Task.status.in_(OPEN_STATUSES), Task.uid != uid).all()
...
def tasks_duplicates_handler(session: Session, args: dict) -> str:
    ...
    session.commit()
    pairs = duplicate_pairs(session, threshold, limit=min(int(args.get("limit", 60)), 200))
    pending = pending_count(session)
    open_count = session.query(Task).filter(Task.status.in_(OPEN_STATUSES)).count()
    return json.dumps({
        "threshold": threshold,
        "pairs": pairs,
...
def tasks_similar_handler(session: Session, args: dict) -> str:
    ...
    return json.dumps({"uid": args["uid"], "similar": similar_tasks(session, args["uid"], limit)})
...
def dupes_tools() -> list[dict]:
    return [
        ...
        CustomTool(
            ...
            input_schema={
                "type": "object",
                "properties": {"uid": {"type": "string"}, "limit": {"type": "integer", "default": 8}},
                "required": ["uid"],
            },
