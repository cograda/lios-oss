...
def open_blockers(session: Session, owner_id: int | None = None) -> dict[str, list[str]]:
    ...
    `owner_id` keeps only the BLOCKED tasks that person owns — "my loops that
    are waiting on something" — whoever owns the blocker. The ledger is
    household-shared, so without it this is everyone's blocked work; the
    Loops summary strip showed Sam the household's count (2026-09-06).
    """
    open_uids = {
    ...
    if owner_id is not None:
        mine = {
            uid for (uid,) in session.query(Task.uid).filter(Task.owner_id == owner_id).all()
        }
        edges = {uid: targets for uid, targets in edges.items() if uid in mine}
    return {
...
def blocking_of(session: Session, owner_id: int | None = None) -> dict[str, list[dict]]:
    """blocker uid -> the open tasks it is holding up, as uid+title.

    The reverse of `open_blockers`, and deliberately unlimited (unlike
    `most_blocking`, which caps at `limit` for the ranked "worth fixing"
    view) — this is what a "tasks I'm sitting on" row wants: not a ranking,
    every open task waiting on this one. Built from the same `_edges()`/
    `open_blockers()` data already computed for this call, so it costs one
    more dict pass, not another query. `owner_id` scopes the blocked side,
    exactly as `open_blockers` does.
    """
    blocked = open_blockers(session, owner_id=owner_id)
    tasks = {t.uid: t for t in session.query(Task).all()}
    out: dict[str, list[dict]] = {}
    for blocked_uid, blockers in blocked.items():
        blocked_task = tasks.get(blocked_uid)
        if blocked_task is None:
            continue
        for b in blockers:
            out.setdefault(b, []).append({"uid": blocked_uid, "title": blocked_task.title})
    return out


def most_blocking(session: Session, limit: int = 10, owner_id: int | None = None) -> list[dict]:
    ...
    `owner_id` ranks only what is holding up that person's own loops.
    """
    blocked = open_blockers(session, owner_id=owner_id)
