...
Adding a source is a one-line change: add it to this set and discovery
starts queueing it as `pending` (mimetype permitting), with no other edit
needed. `gmail` joined 2026-09-07 when `ingest.py` gained a download path
(`google_mail`'s `fetch_attachment`, via the `mail.query` capability); rows
queued `unsupported` for the source before that are re-evaluated by
`scan.reevaluate_unsupported_gmail_rows` / `app.scripts.reevaluate_gmail_attachments`.
"""

from __future__ import annotations

# Sources `ingest.py` can actually download + parse today.
SUPPORTED_INGEST_SOURCES: frozenset[str] = frozenset({"whatsapp", "gmail"})
