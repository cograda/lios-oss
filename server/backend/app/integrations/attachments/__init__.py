...
The flow is always user-gated:
  ...
  3. `ingest` downloads, parses via the corpus parsers, and embeds into
     `historical_documents` with source_type='wa_attachment' — WhatsApp
     only for now; Gmail rows scan in but don't ingest yet (no download path
     wired to `users.messages.attachments.get`).
