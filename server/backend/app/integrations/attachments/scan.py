...
parser — anything else lands 'unsupported' with a reason. Gmail is not in
that set today, so every Gmail attachment is 'unsupported' regardless of
mimetype; it stays a real, undeleted row, just not queued as actionable. No
downloads happen here either way — that's user-gated in `ingest.py`.
...
def _discovery_status(source: str, mime_type: str | None) -> tuple[str, str | None]:
    ...
    consumed if approved", and a Gmail row can't be, yet.
    """
