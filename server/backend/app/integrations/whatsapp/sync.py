...
def _delete_stale(
    ...
    *,
    ...
    `user_id`, when given, confines the pass to that owner's rows: `keep` was
    built from a run that only walked that owner's chats, so every other
    owner's row is absent from it and would otherwise read as stale.
...
def embed_messages(
    ...
    `user_id` restricts the pass — chat discovery AND the two stale-cleanup
    passes — to one owner. The `whatsapp_embed` MCP tool passes the caller's,
    so one user's tool call no longer re-cuts the other user's private chats
    (2026-09-06 scoping audit). `None` is the unbound scheduled sync and
    stays household-wide. The cleanup passes MUST share the scope: they
    delete every row whose key is absent from this run's `all_new_keys`, so
    a run that only walked user 1's chats would otherwise wipe user 2's.
    ...
    chat_q = session.query(WhatsAppMessage.user_id, WhatsAppMessage.chat_id).filter(
        WhatsAppMessage.body.isnot(None)
    )
    if user_id is not None:
        chat_q = chat_q.filter(WhatsAppMessage.user_id == user_id)
    chat_keys = (
        chat_q
    ...
    stale_embedded = _delete_stale(
        session, Embedding, all_new_keys, statuses=None, user_id=user_id,
    )
    if stale_embedded:
    ...
    stale_queued = _delete_stale(
        session, EmbeddingQueue, all_new_keys, statuses=("pending",), user_id=user_id,
    )
    if stale_queued:
