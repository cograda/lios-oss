...
MANIFEST = IntegrationManifest(
    ...
    oauth=None,
    # `system`'s daily-brief Pulse section wants it (issue #195) — merging
    # Strava alongside `health.query`'s workouts so a Strong/HealthKit sync
    # gap doesn't produce a confidently-wrong "none in the last week" when
    # Strava independently has the activity. See facade.py.
    provides=["strava.query"],
    depends_on=[],
)
