...
MANIFEST = IntegrationManifest(
    ...
    routes=[],
    config_schema={
        "sheets_owner_account": ConfigFieldSpec(
            type="str", required=False,
            description="Google account whose OAuth token owns/writes the exported Sheet.",
        ),
    ...
    # The Sheets export needs spreadsheets+drive.file scopes on whichever
    # Google account is sheets_owner_account — declared here since `sheets`
    # itself has no manifest (see chunk 1.1) and snags is its one consumer.
