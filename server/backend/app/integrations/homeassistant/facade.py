...
from app.integrations.homeassistant.tools import (
    ...
    handle_history,
    handle_home_status,
    prune_churn as _prune_churn,
    recent_churn as _recent_churn,
)
...
class HomeAssistantFacade:
    ...
    def recent_churn(
        self, session: Session, hours: int = CHURN_WINDOW_HOURS
    ) -> dict[str, Any]:
        """Entities added/removed from HA in the last `hours` (Wave 5.9).

        Backs `handle_home_status`'s `attention.churn` block and is the
        one place another integration should read entity churn from —
        never `app.integrations.homeassistant.models.HAEntityChurn`
        directly (capability-boundary rule, `tests/test_capability_boundaries.py`).
        """
        return _recent_churn(session, hours)

    def prune_churn(self, session: Session, cutoff: "datetime") -> int:
        """Delete ha_entity_churn rows older than `cutoff`.

        Called from `app.plugin.kernel_jobs`'s daily prune sweep — see that
        module for the retention window and rationale.
        """
        return _prune_churn(session, cutoff)

    def entity_state(self, session: Session, entity_id: str) -> dict[str, Any] | None:
        """The latest known state + freshness timestamps for one entity, or
        None if it's not (yet) in `ha_entities`.

        Added for issue #155 (weather's outdoor-sensor override): a caller
        that wants one live reading, not `handle_entity`'s history payload.
        Returns raw values — deciding what counts as "fresh enough" and
        "not unknown/unavailable" is the caller's call, not this facade's.

        `last_updated` (added for issue #167) is HA's own "have I heard
        anything from this entity" timestamp — distinct from `last_changed`,
        which only moves when the state *value* changes. A caller checking
        for a frozen/dead publisher wants `last_updated`; `None` means the
        row predates this column (or was written by a path that hasn't
        been backfilled) and staleness can't be judged from it.
        """
