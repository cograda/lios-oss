...
# Window for the churn summary surfaced in `handle_home_status`'s `attention`
# block (Wave 5.9) — matches the sync cadence's natural "since yesterday"
# question, same as the offline/low-battery lists it sits alongside.
CHURN_WINDOW_HOURS = 24
# Cap on the removed-entity-id list surfaced inline, for the same reason as
# `_OFFLINE_LIST_CAP` — a fleet-wide churn event (e.g. a device re-added under
# a new integration) shouldn't bloat every home_status response.
_CHURN_LIST_CAP = 30
...
def _churn_to_dict(c: HAEntityChurn) -> dict[str, Any]:
    return serialize(
        c,
        ["entity_id", "event", "domain", "friendly_name", "last_state", "at"],
        transforms={"at": iso_or_none},
    )


def prune_churn(session: Session, cutoff: datetime) -> int:
    """Delete ha_entity_churn rows older than `cutoff`. Returns rows deleted.

    Called from `app.plugin.kernel_jobs` via the facade — kernel code may
    not import this package's models directly (`test_kernel_import_guard.py`),
    so the delete itself lives here rather than in the kernel job.
    """
    deleted = (
        session.query(HAEntityChurn)
        .filter(HAEntityChurn.at < cutoff)
...
def recent_churn(session: Session, hours: int = CHURN_WINDOW_HOURS) -> dict[str, Any]:
    """Entity churn (added/removed) over the last `hours` — the surface for
    `HomeAssistantFacade.recent_churn()` and `handle_home_status`'s
    `attention.churn` block (Wave 5.9).

    Counts are exact (a plain `COUNT(*) ... GROUP BY event`); the `removed`
    list of individual rows is capped at `_CHURN_LIST_CAP` for the same
    reason `handle_home_status`'s offline list is capped — a fleet-wide
    churn event shouldn't bloat every caller of this.
    """
    since = datetime.now(timezone.utc) - timedelta(hours=hours)
    counts = dict(
        session.query(HAEntityChurn.event, func.count(HAEntityChurn.id))
        .filter(HAEntityChurn.at >= since)
        .group_by(HAEntityChurn.event)
        .all()
    )
    removed_rows = (
        session.query(HAEntityChurn)
        .filter(HAEntityChurn.at >= since, HAEntityChurn.event == "removed")
        .order_by(HAEntityChurn.at.desc())
        .limit(_CHURN_LIST_CAP)
        .all()
    )
    return {
        "hours": hours,
        ...
        "removed": [_churn_to_dict(c) for c in removed_rows],
    }
    ...
    return json.dumps({
        **_freshness(session),
        ...
        "attention": {
            ...
            # Entity churn (Wave 5.9) — what appeared/disappeared, not just
            # a count. `entity_count`/`offline_count` above can only say a
            # number changed; this says what changed and names removals.
            "churn": recent_churn(session),
        },
    }, indent=2)
...
def handle_entities(session: Session, arguments: dict[str, Any]) -> str:
    ...
    # `pattern` is the canonical name (matches the schema + description); `query`
    # is accepted too for back-compat with any existing caller using the older
    # name. Neither is silently dropped if the other is present. See issue #172
    # — a schema/handler name mismatch here meant every call was unfiltered.
    substring = arguments.get("pattern") or arguments.get("query")
    if substring:
        like_pattern = f"%{substring}%"
        q = q.filter(
            HAEntity.entity_id.ilike(like_pattern)
            | HAEntity.friendly_name.ilike(like_pattern)
        )
...
def handle_history(session: Session, arguments: dict[str, Any]) -> str:
    ...
    # `hours` is a finer-grained alternative to `days` for a shorter look-back
    # window. Previously undeclared in the schema, so a caller passing `hours`
    # (e.g. `hours=10`) got silently dropped and the response echoed the
    # `days` default instead — issue #172's second smell. Whichever unit was
    # actually asked for is the one echoed back below, so the response never
    # again names a window the caller didn't request.
    if "hours" in arguments:
        hours = min(int(arguments["hours"]), 90 * 24)
        since = datetime.now(timezone.utc) - timedelta(hours=hours)
        window: dict[str, int] = {"hours": hours}
    else:
    ...
    return [
            ...
            input_schema={
                ...
                "properties": {
                    ...
                    "pattern": {
                        "type": "string",
                        "description": (
                            "Substring match (case-insensitive) on entity_id or "
                            "friendly name, e.g. 'shed' or 'washing_machine'. "
                            "`query` is accepted as an alias for back-compat."
                        ),
                    },
                    "query": {
                        "type": "string",
                        "description": "Deprecated alias for `pattern`. Use `pattern` instead.",
                    },
                    "limit": {
                        "type": "integer",
        ...
        CustomTool(
            ...
            description=(
                ...
                "last N days (or N hours, for a shorter window), with counts "
                "per state — answers questions like 'how many washes this "
                "week' or 'when did the dishwasher last run'. Only non-numeric "
                "transitions are recorded (appliance cycles, switches, "
                "presence — not every temperature tick)."
            ),
            input_schema={
                "type": "object",
                "properties": {
                    ...
                    "days": {
                        ...
                        "description": (
                            ...
                            "Ignored if `hours` is also given."
                        ),
                        "default": 7,
                    },
                    "hours": {
                        "type": "integer",
                        "description": (
                            "Look-back window in hours, for a shorter window "
                            "than a whole day (max 2160 = 90 days). Takes "
                            "precedence over `days` if both are given."
                        ),
                    },
                    "limit": {
                        "type": "integer",
