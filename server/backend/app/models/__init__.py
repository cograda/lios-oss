...
# The runs ledger (S5.1; absorbed tool_calls Wave 5.1) — one row per
# scheduled job / tool call / manual run. Kernel-owned for the same reason
# as ai_usage/algo above: the writer (app.services.runs) is called from
# app.scheduler for every registered job (and from app.plugin.dispatch for
# every tool call), so it has to be importable from anywhere a job lives
# without creating an integration dependency cycle. See app/models/runs.py.
from app.models.runs import Run

# Dashboard sign-in sessions (2026-09-06 — one credential: the per-user
# bearer). Kernel-owned: read by the middleware in app/main.py.
