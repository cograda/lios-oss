...
This table does not bring that back. A grant is:
  ...
  **optionally folder-scoped** (2026-09-06) — `folders` is a JSON list of
  vault-relative prefixes (`["Household/", "People/"]`); NULL means the
  whole vault, which is what the `agent` grantee below needs. Enforced at
  one chokepoint in the query layer, `app/services/vault_scope.py`, never
  per tool.

🔑 **Why folders exist.** Alex decided on 2026-09-06 that Sam should read
the renovation and household material in his vault as **shared context
rather than holding a copy** — a copy is a second editable thing that drifts,
which is the failure the monorepo and the single vault-per-user rule both
exist to prevent. But his vault also holds `Health/`, `Notes/`, `Blog/` and
`Daily Notes/`, and those must **never** be in her scope. A whole-vault
grant cannot express that, so a grant now names the folders it opens, and
everything outside them stays exactly as private as it was with no grant at
all.
...
class VaultReadGrant(Base):
    ...
    # Vault-relative folder prefixes this grant opens, each normalised to a
    # trailing slash (`app.services.vault_scope.normalise_folders`). NULL is
    # the whole vault. An empty list is never stored — the service layer
    # rejects it, because `[]` reading as "everything" is the widening
    # failure. Matched by case-sensitive prefix on `vault_chunks.path` and
    # `embeddings.source_id` at the query chokepoint.
    folders: Mapped[list[str] | None] = mapped_column(JSONB, nullable=True)
