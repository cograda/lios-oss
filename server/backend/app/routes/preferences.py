...
unmasked. These routes sit on the session-gated `/api/*` surface like the
rest of the dashboard; a person may read and write their OWN preferences,
and only an admin may touch another user's (2026-09-06).
"""
...
def _require_self_or_admin(caller: User, user_id: int) -> None:
    """A non-admin may only act on their own `user_id`."""
    if caller.is_admin or caller.id == user_id:
        return
    raise HTTPException(403, "Admin only for another user's preferences")
...
async def get_user_preferences(
    user_id: int, session: SessionDep, caller: User = Depends(current_ui_user),
):
...
async def put_user_preferences(
    ...
    # confusing right after someone changes a setting and re-runs /checkin.
