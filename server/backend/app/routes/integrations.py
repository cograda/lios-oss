"""Integration management routes — status, manual sync triggers, detail.

`GET /{name}/history` (standalone recent-sync-history endpoint) was removed
2026-09-05 (Wave 5.10 CI hygiene) — dead: no caller in `frontend/`, `client/`,
or `apps/loops/backend`, and no test exercised it. `GET /{name}/detail` already
returns "status + recent history" as part of its own payload (see its
docstring below), which is what the dashboard actually reads.
"""
...
async def put_integration_enabled(
    name: str, request: Request, _admin: User = Depends(require_admin),
):
...
async def integration_tools(name: str, hours: int = 24):
    ...
    Call counts come from the `runs` ledger's `kind="tool_call"` rows
    (Wave 5.1 absorbed the standalone `tool_calls` table) — the same rows
    `/system/tool-stats` reads (2.5) — reused here rather than duplicated,
    scoped to this integration's tool names.
    """
    ...
    if names:
        ...
        with db.session() as session:
            rows = (
                session.query(Run.name, func.count(Run.id))
                .filter(Run.kind == "tool_call", Run.name.in_(names), Run.started_at >= cutoff)
                .group_by(Run.name)
                .all()
            )
...
async def trigger_sync(name: str, _admin: User = Depends(require_admin)):
...
async def gmail_backfill(
    after_date: str = "2021/01/01", _admin: User = Depends(require_admin),
):
...
async def lastfm_backfill(_admin: User = Depends(require_admin)):
...
async def gmail_embed(_admin: User = Depends(require_admin)):
    ...
    with db.session() as session:
        try:
            # Deliberately unscoped: an admin route with no bound user; the
            # pass attributes rows per owner and uses each owner's own token.
...
async def historical_corpus_ingest(
    limit_per_type: int | None = None, _admin: User = Depends(require_admin),
):
...
async def get_integration_config(name: str, _admin: User = Depends(require_admin)):
