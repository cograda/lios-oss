...
`reclean` and `fill-space` (never `drain`/`prune-blank`) run inside
`app.services.runs.record_run(kind="script", name="embedding_reembed")` — see
that module and `system/tools.py::_backfill_run_in_progress` (Wave 5.2). This
is the one signal `system_alerts`' `index_state` axis has for `fill-space`
specifically: it vectorises directly, bypassing `EmbeddingQueue` entirely, so
`queue_pending` never moves and a pending-count threshold alone would report
"not rebuilding" for a run that can take hours.
"""
...
BACKFILL_RUN_NAME = "embedding_reembed"
...
def main() -> int:
    ...
    with get_db().session() as session:
        if args.cmd == "reclean":
            ...
            with record_run("script", BACKFILL_RUN_NAME, trigger="cli") as run:
...
def drain(session, batch_size: int, expected: int) -> int:
    ...
    `spaces="all"` (2026-09-07): `process_queue`'s own default became
    `"primary"` when the live worker stopped paying for every active space
    on every cycle (see `app.services.embedding`'s module docstring). A
    reclean changed the *text*, which invalidates every space's vector for
    a chunk, not just the primary's — this drain exists specifically to
    push a reclean through before the next reader sees half-old-text
    search results, so it must still write every space inline rather than
    leaving the others for the nightly `embedding_space_backfill` to close.
    """
    from app.services.embedding import EmbeddingService

    total = 0
    while True:
        n = EmbeddingService.process_queue(session, batch_size=batch_size, spaces="all")
        if not n:
            break
        total += n
