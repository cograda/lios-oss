...
PREFERENCES: dict[str, PreferenceSpec] = {
    ...
    # Defaults halved 2026-09-02, then raised again 2026-09-07 (25->40,
    # 30->120): Alex — "most WhatsApp messages are short", so the per-message
    # character cost is lower than the original 50/50 sizing assumed, and the
    # brief's own file-spool path (see `system_daily_brief`'s tool
    # description) already handles a payload too big to inline. `brief.py`'s
    # `_COMMS_POOL_CEILING` moved from 100 to 600 alongside this so the
    # breadth-first window cap (`_slice_window`) isn't clamped below the new
    # display caps before it gets to apply them.
    "comms.mail_limit": PreferenceSpec(
        type="int",
        default=40,
        description=(
            "Safety cap on how many recent emails the brief pulls for "
            "triage. The window is `_meta.lookback_start`; this cap only "
            "bites when the window holds more than it allows, and the "
            "brief reports `truncated`/`window_covered` when it does."
        ),
        group="comms",
    ),
    "comms.whatsapp_limit": PreferenceSpec(
        ...
        default=120,
        description=(
            "Safety cap on how many recent WhatsApp messages the brief "
            "pulls for triage. The window is `_meta.lookback_start`; this "
            "cap only bites when the window holds more than it allows, and "
            "the brief reports `truncated`/`window_covered` when it does."
        ),
        group="comms",
    ),
}
