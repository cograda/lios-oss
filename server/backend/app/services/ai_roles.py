...
#: don't.
ROLES = frozenset({"stt.memo", "vision.inbox", "vision.watch", "embed.corpus"})
...
def resolve_vision_watch(*, fallback: str | None = None) -> RoleBinding:
    """`vision.watch` — the signals watcher framework's compare() calls
    (`app/integrations/signals/watchers/`), added with the milk watcher
    (2026-09-11).

    Storage: `vision.watch_model` (`app/integrations/vision/manifest.py`) —
    a SEPARATE field from `vision.model` (which serves `vision.inbox`),
    even though both currently default to the same measured model. Kept
    separate deliberately: `vision.inbox` triages one image at a time,
    off a queue, and a bad call just delays a description; `vision.watch`
    compares multiple images per call and gates a phone push, so if a
    future measurement wants a stronger (or differently-tuned) model for
    comparison-and-verdict versus single-image description, that's a
    one-field config change rather than a role that was silently serving
    two different jobs off one knob.

    `fallback` exists for the same kernel/integration import-boundary reason
    `resolve_vision_inbox` documents — the caller that legitimately knows
    its own manifest default (`vision/facade.py`) supplies it.
    """
    from app.plugin.config_store import plugin_config

    cfg = plugin_config("vision")
    model = (getattr(cfg, "watch_model", "") or fallback or "").strip()
    if not model:
        raise RoleNotBoundError(
            "role 'vision.watch' has no binding — vision.watch_model is empty. "
            "Set it via PUT /api/integrations/vision/config."
        )
    _warn_if_unrated("vision.watch", model)
    return RoleBinding(role="vision.watch", provider="google", model=model)
...
_SINGLE_RESOLVERS = {
    ...
    "vision.watch": resolve_vision_watch,
}
