...
import { useDashboard, useIntegrations, useClients, useSystemInfo, useSyncStream } from '@/hooks/use-api'
...
export default function Dashboard() {
  // Live sync-state stream (issue #141) — invalidates `useClients` /
  // `useSystemAlerts` on daemon connect/disconnect/heartbeat instead of
  // waiting out their own poll interval. Mounted once, here, since this is
  // the page that renders both consumers (ClientStatusCard, SystemAlertsPanel).
  const { connected: liveSync } = useSyncStream()
  const { data, error, isLoading } = useDashboard()
  const { data: intData } = useIntegrations()
  const { data: clientData } = useClients(liveSync)
  ...
  return (
    <div className="space-y-6">
      ...
      <SystemAlertsPanel live={liveSync} />
