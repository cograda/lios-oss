...
function ActiveAlertsList({ live }: { live: boolean }) {
  const { data, isLoading } = useSystemAlerts(live)
...
export function SystemAlertsPanel({ live = false }: { live?: boolean }) {
  const { data } = useSystemAlerts(live)
  const alertCount =
