...
@dataclass
class AlertsConfig:
    """Local, server-independent alerting — see `server_alert.py`.

    Off-box detection of the case where the server (and with it every other
    household monitor, since they all run on the same Proxmox cluster) has
    gone silent. `server_silent_minutes = 0` disables the detector entirely.
    """

    server_silent_minutes: int = 20
    server_silent_refire_hours: float = 6
...
def load_config() -> ClientConfig:
    ...
    if "alerts" in data:
        ...
        config.alerts = AlertsConfig(
            server_silent_minutes=int(a.get("server_silent_minutes", config.alerts.server_silent_minutes)),
            server_silent_refire_hours=float(a.get("server_silent_refire_hours", config.alerts.server_silent_refire_hours)),
        )
