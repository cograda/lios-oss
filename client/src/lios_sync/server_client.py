...
# The SSE stream's read timeout. sse-starlette pings every 15 s by default, so
# four missed pings means the connection is dead, not quiet. See open_event_stream.
...
class ServerClient:
    ...
    def heartbeat(self, task_health: dict | None = None) -> dict:
        ...
        `server_time`. Verifies credentials.
        """
    ...
    def get_commands(self) -> dict:
        ...
        "retired": ["<filename>.md", ...]}`. `retired` (added 2026-09-07,
        alongside the `/daily-note` → `/kickoff` rename and `/triage`'s
        retirement) names command files this client may have previously
        fetched under a now-dead slug — see `_refresh_commands` in
        `daemon.py` for the delete-only-those-names cleanup it drives.
    ...
    def open_event_stream(self):
        ...
        """
        # Build a fresh client for the long-lived stream. The read timeout is
        # NOT None: the server (sse-starlette) writes a `: ping` comment every
        # 15 s, so a stream that is silent for _STREAM_READ_TIMEOUT is dead,
        # whatever the socket says. With read=None the daemon sat 33 hours in
        # one `next()` on a half-open socket after a container replacement
        # (2026-09-05) — the server had gone, no FIN ever arrived, and nothing
        # else in the daemon closes this stream. The timeout raises out of
        # `iter_lines`; the events loop backs off and reconnects.
        ...
        with httpx.Client(
            ...
            timeout=httpx.Timeout(connect=10.0, read=_STREAM_READ_TIMEOUT, write=10.0, pool=10.0),
        ) as stream_client:
            ...
            try:
                with stream_client.stream("GET", "/api/v1/events") as resp:
                    ...
                    for line in resp.iter_lines():
                        if line.startswith(":"):
                            # SSE comment — the server's keepalive ping. Not an
                            # event, but it IS proof the stream is alive, so
                            # surface it: the daemon beats its supervisor on
                            # every yielded item, and a task whose liveness
                            # only advanced on real events read as "silent for
                            # a day" on a perfectly healthy, quiet stream.
