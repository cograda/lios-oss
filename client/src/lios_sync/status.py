...
def show_status():
    ...
    try:
        ...
        alert = data.get("server_alert") or {}
        alert_state = alert.get("state", "ok")
        if alert_state == "ok":
            _ok("Server alert: none")
        elif alert_state == "server_silent":
            _warn(f"Server alert: server silent (threshold {alert.get('threshold_minutes')}m)")
        elif alert_state == "auth_invalid":
            _err("Server alert: this Mac's token was rejected (401)")
        elif alert_state == "network_down":
            _warn("Server alert: this Mac's own network looks down")
    except Exception:
