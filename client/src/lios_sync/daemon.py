...
from lios_sync.server_alert import ServerSilenceMonitor
...
async def run_daemon():
    ...
    # Off-box "server silent" detector (see server_alert.py) — the inverse of
    # the per-host liveness work happening inside core: every monitor the
    # household has runs ON the Proxmox cluster, so if that cluster or its
    # docker host dies, this Mac is the only thing left that can say so.
    silence_monitor = ServerSilenceMonitor(
        threshold_minutes=config.alerts.server_silent_minutes,
        refire_hours=config.alerts.server_silent_refire_hours,
    )
    ...
    try:
        ...
        await asyncio.to_thread(silence_monitor.record_result, None)
        logger.info("Server connected")
        await asyncio.to_thread(_refresh_commands, server_client, config)
    except Exception as e:  # noqa: BLE001
        server_client.mark_disconnected()
        await asyncio.to_thread(silence_monitor.record_result, e)
...
async def _handle_server_event(
    ...
    if event_type == "eventkit_command":
        ...
        pass  # server ping; the events loop already beat on it. Not logged: one every 15 s.
    else:
...
def _refresh_commands(server_client: ServerClient, config) -> None:
    ...
    try:
        ...
        # Belt and braces for the specific retired names the server names
        # explicitly (2026-09-07: `daily-note.md`, `triage.md`) — the loop
        # above already drops them as a side effect of "not in `keep`", but
        # this makes the intent explicit and survives a future refresh path
        # that doesn't do a full directory diff. Only ever deletes a name
        # the server itself names as retired — never anything else.
        for retired_name in payload.get("retired", []) or []:
            retired_path = commands_dir / Path(retired_name).name
