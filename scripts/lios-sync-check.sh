#!/usr/bin/env bash
# lios-sync-check.sh — preflight liveness check for the lios-sync daemon.
#
...
#
# Usage:
#   scripts/lios-sync-check.sh           # check, auto-heal if needed
#   scripts/lios-sync-check.sh --no-heal # check only, never restart
#
set -uo pipefail
...
echo "⚠️  lios-sync daemon unhealthy: $(cat /tmp/.comar_health_err 2>/dev/null)" >&2
