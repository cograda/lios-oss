...
# wiring. `file_size`/`file_created` are still simply ignored by
# `routes/inbox.py`, so they are left alone rather than surgically removed:
# every deletion is a chance to break an attachment reference, and an ignored
# key costs nothing.
#
# `comar` and `email` are NOT ignored any more (lios#198/#191, fixed
# 2026-09-10): `comar` carries the Dictator Shortcut's "file in Comar?
# Yes/No" answer, as a flat top-level key (not nested under a `metadata`
# object). A falsy answer now makes `routes/inbox.py` transcribe/describe
# the capture, email it to the caller, and discard it — no filing, no push,
# nothing left in the inbox tree. Before this fix the key was read by
# nobody, so answering "No" made no observable difference; two real work
# memos leaked into the inbox this way on 2026-09-10. `email` is still
# unread server-side — delivery is routed by the caller's resolved user_id,
# not a request-supplied address — but it is no longer accurate to call it
# ignored-alongside-comar, so it gets its own mention here.
