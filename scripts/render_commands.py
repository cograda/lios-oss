...
versioned sources are:
  ...
  * generated commands — `server/backend/app/prompts/templates/*.md.j2`,
    rendered per-user by `app.prompts.commands`. These carry a
    `<!-- GENERATED -->` header. Never hand-edit the delivered file; edit the
    template and re-run this script. Thirteen as of 2026-09-06, when the
    eight shared hand-authored commands were folded into the registry.
  * static commands — `commands/*.md` at the element root. Hand-authored,
    Alex-only (finance, import-finance, linkedin, listening), copied through
    verbatim. Edit these directly.

Which is which — and who gets what — is `COMMAND_TABLE` in
`app/prompts/commands.py`; this script iterates the table rather than the
directory, so a static file the table does not name is an error, not a
silent extra.

Sam's equivalent set is delivered by the installer from
`GET /api/v1/commands` into her `~/lios/` working folder. That path is
...
which asserts Alex's render of every template matches `tests/snapshots/commands/*.md`.
...
from app.prompts.commands import (  # noqa: E402
    render_command_set,
...
# This script delivers for exactly one user.
USER = "alex"


def static_command_paths() -> list[Path]:
    """The static files COMMAND_TABLE says Alex gets — and a check that the
    directory holds nothing else, so a file added without a table row fails
    here rather than being silently skipped."""
    expected = {f"{slug}.md" for slug in static_commands_for_user(USER)}
    on_disk = {
        p.name for p in STATIC_DIR.glob("*.md") if p.name not in STATIC_NON_COMMANDS
    }
    if on_disk != expected:
        raise SystemExit(
            f"commands/*.md on disk {sorted(on_disk)} does not match COMMAND_TABLE's "
            f"static rows for {USER} {sorted(expected)} — add the table row "
            "(app/prompts/commands.py) or move the file."
        )
    return [STATIC_DIR / name for name in sorted(expected)]
...
def collect() -> dict[str, str]:
    ...
    """
    out: dict[str, str] = dict(render_command_set(USER))
    ...
    for path in static_command_paths():
        if path.name in out:
            raise SystemExit(
                ...
                f"the source of truth for every template command."
            )
