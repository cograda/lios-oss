...
Two ways in. **Flagged first:** Alex marks tasks in the tasks app with the `#youdoit` tag
(the ⚡ on a row, or `y`), which is him saying "you do this one". Those are not candidates to
score — they are the queue. **Then the scan**, scoped by the argument or over the open
backlog, for tasks he has not flagged but that fit.
...
1. `tasks_query(tag="#youdoit", limit=100)` — the flagged queue. Every one of these gets a
   prompt (Step 5) unless it is blocked, in which case say what blocks it.
...
For each approved task, `tasks_update(uid=..., description=...)` with the existing description
**plus** an appended block — never a replacement: the date, the target, one line on what the
prompt covers, one line on what it deliberately leaves to Alex. Leave the `#youdoit` tag on;
Alex removes it when the output is in.
